...
backend, no cloud storage, no localStorage state (one sanctioned cosmetic
exception: the theme override, decision 20): wipe your browser, re-grant
...
- **Required vs optional is a real semantic.** Required-missing → the mod
  ...
  dependency is a real failure mode. Hence toybox's rule: **`recommends` are
  never auto-installed, but when present their version range is validated**
  (violations surface as warnings). toybox's `required`/`recommends` map onto
  StarMap's required/optional at load time.
...
[who_can_reference]           # REQUIRED: who may declare references to this mod
include = [".*"]              # regex on referencing mod ids; full-match, case-insensitive
exclude = []                  # exclusions always win; ".*" include = anyone (the default)
```
...
# [[required]]                # hard dependency: resolver-enforced, auto-installed
# id = "SomeLib"
# range = "^2.1"
# description = "Provides the physics solver this mod builds on."
...
CI compiles the TOML tree into a maven-style layout: one small central
discovery document plus convention-based per-mod files fetched on demand.

- **`v1/index.json`** — the central catalog: identity, summaries, tags,
  authors, releases, dependencies, artifacts with digests + GitHub API asset
  URLs. Everything search and resolution need in one fetch — and nothing
  bulky: readmes and per-file manifests live outside it, so the index stays
  small no matter how large the catalog grows. Schema-versioned
  (`schema: 1`); breaking changes ship as `v2/` alongside `v1/`.
- **`v1/mods/<slug>/readme.md`** — the CURRENT rich markdown readme,
  lazy-fetched by the app when a mod's detail page opens (the index carries
  its convention-generated `readmePath`).
- **`v1/mods/<slug>/readmes/<version>.md`** — the readme snapshot as of each
  release's registration commit, derived from the index repo's git history
  at compile time (each compiled release carries its `readmePath`). This
  completes **the vendoring convention**: every per-version datum lives at
  `v1/mods/<slug>/<kind>/<version>[.<artifactKey>].<ext>` (kinds today:
  `readmes/`, `manifests/`, `artifacts/`); future per-version data must
  follow the same shape.
- **`v1/mods/<slug>/manifests/<version>.<artifactKey>.json`** — the per-file
...
- **`v1/mods/<slug>/artifacts/<version>.<artifactKey>.zip`** — **mirrored
  artifact bytes** for the mod's newest `mirror_versions` releases (a
  registration-level, admin-reviewed setting; the Pages site budget is
  ~1 GB, so mirroring is rationed). This exists because GitHub release
  downloads are not browser-fetchable (no CORS on the redirect chain —
  verified empirically); the mirror is same-origin with the app under the
  shared domain, so it always works, mobile included. CI already downloads
  every artifact to verify digests, so mirroring adds no extra bandwidth
  (bytes are kept in a content-addressed cache). The app's download strategy
  is mirror → GitHub API → direct → verified local file.
...
- **References — `required` and `recommends`:** a release declares two lists
  of rich reference entries (`id` + semver `range` + optional publisher
  `description` ≤ 400 chars, shown in chips, hints, and failure derivations).
  `required` drives resolution — hard edges, auto-installed, blocking. A
  release may not reference itself, list a mod twice, or put one mod in both
  lists. `recommends` are never auto-installed (StarMap optional semantics);
  when a recommended mod IS present its version is validated (warning, not
  error — surfacing the assembly-sharing hazard), and the cart surfaces
  not-selected recommendations as one-click "add" hints.
- **`who_can_reference` — reference permissioning:** references are
  self-declared, which caused real drama in CKAN-land (mod packs recommending
  mods against those authors' wishes). Every registered mod must declare
  `[who_can_reference]`: `include`/`exclude` regex arrays matched (full-match,
  case-insensitive) against the _referencing_ mod's id; exclusions always win;
  the expected default is include-all. Enforced when the index is compiled: a
  disallowed `recommends` is silently filtered from the published index; a
  disallowed `required` fails validation outright (a hard dependency cannot
  ship against the target author's wishes — humans mediate). The app never
  sees disallowed references, so no client-side handling exists or is needed.
...
are the durable interface. Theming is cat-appropriate (🧶) and three-state:
light/dark palettes with a header toggle cycling system → light → dark; the
effective theme is stamped on `<html data-theme>` by a pre-paint inline script
(no wrong-theme flash) and persistence follows decision 20.
...
(required references pulled in, recommends not), every artifact is downloaded
...
- **Unit/integration (node):** every core module — semver/ksa-version
  ...
  fail-closed, token, progress), resolver (required refs, recommends, conflicts,
...
| 22  | Platform is a cart-wide, user-retargetable setting (one mods folder = one game install); OS detection is only the default. Release rows use a split button: primary = sensible default platform, dropdown = any platform the release ships | Detection is a guess — Proton on Linux wants windows artifacts, shared drives and bundles-for-friends exist; nothing should be blocked by the browser OS                                           |
